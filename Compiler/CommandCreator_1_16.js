...
var CommandCreator = 
{
	...
	incrementSetblockVars : function()
	{
		...
		switch(CommandCreator.currentCommandBlock.direction)
		{
			case "east":
				...
				{
					CommandCreator.currentCommandBlock.direction = CommandCreator.UP_DIRECTION_VALUE;
			...
			case "up":
				...
				if(CommandCreator.currentCommandBlock.x == CommandCreator.currentCommandModule.innerHighX)
				{
					CommandCreator.currentCommandBlock.direction = CommandCreator.WEST_DIRECTION_VALUE;
					CommandCreator.currentDirectionChanged = true;
				}
				else if(CommandCreator.currentCommandBlock.x == CommandCreator.currentCommandModule.innerLowX)
				{
					CommandCreator.currentCommandBlock.direction = CommandCreator.EAST_DIRECTION_VALUE;
					CommandCreator.currentDirectionChanged = true;
				}
	...
	buildSetblockCommand : function(x, y, z, direction, type, conditional, auto, trackOutput, executeAs, command)
	{
		...
		var conditionalStr = conditional? "true" : "false"
		...
		if(executeAs != "")
			command = util.format("/execute as %s %s", executeAs, command);
		...
		var setblock = util.format(Templates.Current.SETBLOCK_COMMAND_FORMAT, 
								   x, y, z,
								   blockName, direction, conditionalStr, JSON.stringify(command), autoString, trackOutputString);
